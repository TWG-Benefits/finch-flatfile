...
export default function processPayment(plan_id: number, finch: FinchRequiredData): CSVRow[] {
    const payDate = finch.payment?.pay_date
    const payPeriodStartDate = finch.payment.pay_period?.start_date
    const payPeriodEndDate = finch.payment.pay_period?.end_date
    const payFrequencyCode = "semi_monthly" // ???

    var test: CSVRow[] = []
    finch.payStatement.body.pay_statements.forEach(paycheck => {
